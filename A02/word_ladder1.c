...
static hash_table_node_t *allocate_hash_table_node(const char *word)
{
...
static hash_table_t *hash_table_create(unsigned int size)
{
  ...
  hash_table->heads= (hash_table_node_t **) calloc(size,sizeof(hash_table_node_t ));
...
void add_new_node(hash_table_t *hash_table,const char *word, unsigned int index)
{
  hash_table_node_t * node;  
  node = allocate_hash_table_node(word);
  hash_table_node_t * current_node = hash_table->heads[index];
  
  if (current_node == NULL) // if is the head
  {
    hash_table->heads[index]=node;
    hash_table->heads[index]->previous=NULL;    
  }
  else // if isn't the head
  {
    do
    {
...
static hash_table_t hash_table_grow(hash_table_t *hash_table)
{
  hash_table_t *hash_table_new;
  unsigned int new_size = hash_table->hash_table_size*10;
  hash_table_new=hash_table_create(new_size);
  

  for (unsigned int line = 0; line < hash_table->hash_table_size; line++)
  {
    hash_table_node_t * current_node = hash_table->heads[line];

    if (current_node != NULL)
    {
      do
      {
        hash_table_new->heads[hash_function(current_node->word,new_size)] = current_node;
        hash_table_new->number_of_entries+=1;
        current_node=current_node->next;                       
      } while (current_node->next != NULL);
    }       
  }
  ...
  free(hash_table);
  hash_table->heads =hash_table_new->heads;
...
static void hash_table_free(hash_table_t *hash_table)
{
  for (unsigned int i = 0; i < hash_table->hash_table_size; i++)
  {
    ...
    if (current_node == NULL)
    {
      free_hash_table_node(current_node);
    }
    else
    {
      do
      {        
        ...
        if (current_node != NULL)
        {
          free_hash_table_node(current_node->previous);
        }               
      } while (current_node->next != NULL);
...
static hash_table_node_t *find_word(hash_table_t *hash_table,const char *word,int insert_if_not_found)
{
  ...
  {
    add_new_node(hash_table,word,i);
    // if is to big 
...
int main(int argc,char **argv)
{
  ...
  hash_table = hash_table_create(100);
  // read words
